...
import type { BookmarkDetail, DeviceInfo, ExportInfo, TreeNode } from "./types";
import {
  btnPrimaryClass,
  btnSecondaryClass,
  cardClass,
  ...
  const load = useCallback(
    async (opts?: {
      ...
      try {
        ...
        const deviceList = await fetchDevices(currentUser.id);
          ...
          <div className="flex items-end gap-3">
            ...
            <div className="ml-4 flex items-center">
              <div ref={menuRef} className="relative">
                <button
                  type="button"
                  className={`${btnSecondaryClass} flex items-center gap-2`}
                ...
                >
                  ...
                  <span className="text-xs">{userId ? "User" : "—"}</span>
                </button>
                {menuOpen && (
                  <div className="absolute right-0 mt-2 w-44 rounded-md border border-border bg-surface p-1 shadow-lg">
                    <button
                      type="button"
                      className="w-full text-left px-3 py-2 text-sm hover:bg-surface/80"
                      onClick={() => {
                        setMenuOpen(false);
                        // placeholder: navigate to stats
                    ...
                    <button
                      type="button"
                      className="w-full text-left px-3 py-2 text-sm text-danger hover:bg-surface/80"
                      onClick={async () => {
                        setMenuOpen(false);
                        try {
                          await fetchLogout();
